...
const Navbar = () => {
  ...
  return (
    ...
    border-gray-288 min-h-[90vh] w-[80px]
     ...
     items-start justify-betwen'>
      <div className='space-y-8 w-full'>
            ...
            <div className='w-full border-t border-gray-200'/>
             {navLinks.slice(1,5).map((link)=>(
              <Link to="/earnings" onClick={()=>{ActiveComponent(link.id)}}>
                <NavItems link={link} key={link.id}/>
              </Link>
      ...
      space-y-4 p-4'>
        ...
        <p className='text-black'>Click the button below to view</p>
          <Link to='/earnings' className='w-full py-2 px-3 bg-sky-600 text-center text-white rounded-lg border-2 hover:bg-white hover:border-sky-600 hover:text-sky-600'>
             Earnings
          </Link>
      </div>
    ...
    return(
      <div onClick={()=>setActiveNav(link.id)}   className={`w-full flex items-center justify-start space-x8
            ...
            border-transparent ${ActiveNav === link.id && 'border-slate-50'}`}>
                <span>{link.icon}</span>
                <h1 className='text-gray-900 group-hover:text-sky-600 xl:flex hidden m-2'>{link.title}</h1>
      </div>
    )
